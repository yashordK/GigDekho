...
export default function HomeScreen() {
  ...
  console.log("HomeScreen Render:", { user: user?.id, profile: !!profile, loading });
  ...
  useEffect(() => {
    console.log("HomeScreen fetchUserStats useEffect triggered:", { user: user?.id, profile: !!profile });
      ...
      {showHowItWorks && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowHowItWorks(false)}>
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative" onClick={e => e.stopPropagation()}>
            <button type="button" onClick={() => setShowHowItWorks(false)} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center bg-slate-100 rounded-full text-slate-500 font-bold">✕</button>
      ...
      <div className="px-4 xl:px-12 w-full mx-auto">
        ...
        <section aria-label="Available Gigs" className="w-full" id="available-jobs">
          ...
          <div className="mb-6 lg:mb-8 flex justify-between items-start">
             ...
             <div className="flex space-x-2">
                <button type="button" className="bg-[#1C1C1C] hover:bg-white/10 p-2 rounded-full text-white/70 transition-colors shadow-sm border border-white/5">
        ...
        {error && (
           <div className="bg-red-50 text-red-600 p-4 rounded-xl text-sm font-bold border border-red-100 mb-4 flex justify-between items-center">
             <span>{error}</span>
             <button type="button" onClick={fetchData} className="underline text-red-700">Retry</button>
           </div>
        )}
        ...
        <div className="hidden lg:block sticky top-24 space-y-6">
                 ...
                 <div className="space-y-5 mt-2 px-6">
                    {trendingGigs.map((trend, i) => (
                      <div key={trend.id} className={`flex items-start ${i > 0 && 'border-t border-white/5 pt-5'}`}>
                         ...
                         <div className="cursor-pointer hover:underline" onClick={() => navigate(`/gigs/${trend.id}`)}>
           ...
           <div className="bg-gradient-to-br from-[#6231d4] to-[#4510b6] rounded-3xl p-6 relative overflow-hidden shadow-lg border border-[#7d4de2]">
             ...
             <button type="button" className="bg-white hover:bg-slate-50 text-[#6231d4] font-bold py-2.5 px-6 text-sm rounded-full transition-colors shadow-sm btn-tap relative z-10">
               Get Invite Link
             </button>
           </div>
