...
export default function ProfileScreen() {
  ...
  if (loading) {
    return <div className="min-h-screen flex justify-center pt-20 bg-[#111111]"><div className="w-8 h-8 border-4 border-slate-200 border-t-primary rounded-full animate-spin"></div></div>;
  }
        ...
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-end sm:items-center sm:justify-center animate-in fade-in">
           <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 pb-12 animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-4 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                 ...
                 <button type="button" onClick={() => setShowSkillsModal(false)} className="p-2 bg-slate-100 text-slate-500 rounded-full btn-tap"><X size={18} /></button>
              </div>
            ...
            {isEditingProfile ? (
              <div className="flex flex-col items-center w-full max-w-[250px] mb-3">
                 ...
                 <div className="flex space-x-2">
                   <button type="button" onClick={() => setIsEditingProfile(false)} className="px-4 min-h-[44px] rounded-full text-xs font-bold bg-white/10 text-white/70 hover:bg-white/20 btn-tap">Cancel</button>
                   <button type="button" onClick={handleSaveProfile} disabled={savingProfile} className="px-4 min-h-[44px] rounded-full text-xs font-bold bg-[#F4511E] text-white shadow-sm flex items-center btn-tap">
              ...
              <>
                <h1 className="text-xl lg:text-3xl font-black text-white mb-1 lg:mb-2 flex items-center tracking-tight">
                  ...
                  <button type="button" onClick={() => setIsEditingProfile(true)} className="ml-2 text-white/40 hover:text-[#F4511E] btn-tap">
                    <Edit2 size={16} />
                  </button>
                </h1>
