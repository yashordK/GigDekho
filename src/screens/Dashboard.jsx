...
export default function DashboardScreen() {
  ...
  return (
    <div className="pb-24 lg:pb-12 bg-background min-h-screen pt-4">
      ...
      <div className="relative w-full pt-12 pb-24 hero-gradient-overlay flex flex-col items-center justify-center text-center px-4 overflow-hidden mb-6">
        ...
        <p className="text-white/80 font-medium text-base lg:text-lg mb-8 max-w-md relative z-10 leading-relaxed">
          Track your ongoing applications and review past events.
        </p>

        {/* Tabs */}
        <div className="flex glass-panel p-1.5 rounded-full w-full lg:w-[400px] shadow-lg relative z-10">
           <button 
             onClick={() => setTab('active')}
             className={`flex-1 min-h-[44px] py-1.5 text-sm font-bold rounded-full transition-all btn-tap ${tab === 'active' ? 'bg-[#00BCD4] text-white shadow-md' : 'text-slate-600 bg-white/50 hover:bg-white flex items-center justify-center'}`}
           >
             Active Gigs
           </button>
           <button 
             onClick={() => setTab('past')}
             className={`flex-1 min-h-[44px] py-1.5 text-sm font-bold rounded-full transition-all btn-tap ${tab === 'past' ? 'bg-[#00BCD4] text-white shadow-md' : 'text-slate-600 bg-white/50 hover:bg-white flex items-center justify-center'}`}
           >
             Past Events
           </button>
        </div>
      ...
      <div className="px-4 xl:px-12 w-full mx-auto">
        <div className="w-full lg:grid lg:grid-cols-2 xl:grid-cols-3 lg:gap-6">
          {loading ? (
            <div className="flex justify-center p-10 col-span-full"><div className="w-8 h-8 border-4 border-slate-200 border-t-primary rounded-full animate-spin"></div></div>
          ) : filteredApps.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 lg:p-12 text-center shadow-sm border border-slate-100 flex flex-col items-center mt-2 col-span-full">
               <div className="w-20 h-20 bg-orange-50 rounded-full flex justify-center items-center text-primary mb-5 border border-orange-100">
                 <Briefcase size={32} />
               </div>
               <p className="text-slate-900 font-black mb-2 text-xl tracking-tight">No {tab} gigs found</p>
               ...
               {tab === 'active' && (
                 <button onClick={() => navigate('/')} className="mt-8 px-8 py-3.5 min-h-[44px] bg-primary text-white text-sm font-black tracking-wide rounded-xl shadow-lg shadow-orange-500/20 hover:bg-orange-600 transition-colors btn-tap">
                   Browse Live Gigs
                 </button>
               )}
            </div>
               ...
               {filteredApps.map(app => {
                 ...
                 return (
                   <div 
                     ...
                     className="bg-white rounded-2xl p-5 lg:p-6 shadow-sm border border-slate-100 flex flex-col btn-tap cursor-pointer group hover:border-primary/30 transition-all hover:shadow-md mt-4 lg:mt-0"
                   >
                     <div className="flex justify-between items-start mb-5">
                        <div className="flex pr-4">
                          ...
                          <div>
                            <h3 className="font-black text-slate-900 text-lg lg:text-xl leading-tight mb-1">{app.gig.title}</h3>
                            <p className="text-slate-500 font-medium text-sm">{app.gig.location_text}</p>
                          </div>
                        </div>
                        <div className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border whitespace-nowrap shadow-sm ${tab === 'active' && app.status === 'accepted' ? 'bg-green-50 text-green-700 border-green-200' : sParams.color}`}>
                          {sParams.label}
                        </div>
                     </div>
                     
                     <div className="flex justify-between items-end mt-auto pt-5 border-t border-slate-50">
                        <div className="flex items-center text-slate-600 text-sm font-bold bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                          <Calendar size={14} className="mr-2 text-primary" /> 
                          {formatRelativeDate(app.gig.event_date)}
                        </div>
                        <div className="flex items-center">
                          <span className="font-black text-accent text-2xl mr-2 tracking-tight">₹{totalPay}</span>
                          <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-orange-50 transition-colors border border-slate-100 group-hover:border-orange-100">
