 ...
 * RLS security audit.
 *
 * Signs in as a real user and attempts writes that should be impossible,
 * so policy regressions get caught instead of sitting silently in production.
 * Every test cleans up after itself.
 *
 *   node scripts/audit-rls.mjs                      # uses .env.local
...
const EMAIL = process.env.AUDIT_EMAIL || env.AUDIT_EMAIL;

if (!URL || !ANON || !SERVICE) { console.error("Missing Supabase env vars."); process.exit(1); }
if (!EMAIL) { console.error("Set AUDIT_EMAIL to an existing user's email."); process.exit(1); }
...
const { data: other } = await admin.from("profiles").select("id").neq("id", me).limit(1).single();
...
// Privilege escalation
{
  ...
  if (escalated) await admin.from("profiles").update({ is_admin: false }).eq("id", me);
}
{
  ...
  if (changed) await admin.from("profiles").update({ city: "Indore" }).eq("id", other.id);
}
{
  const { data, error } = await user.from("profiles").update({ id_verified: true }).eq("id", me).select("id, id_verified");
  const selfVerified = !error && data?.[0]?.id_verified === true;
  check("cannot self-award a verification badge", !selfVerified);
  if (selfVerified) await admin.from("profiles").update({ id_verified: false }).eq("id", me);
}
...
{
  const { error } = await user.from("profiles").update({ city: "Indore" }).eq("id", me);
...
console.log(
  failures === 0
    ...
    : `\n\x1b[31m${failures} check(s) failed — run supabase/migrations/010_rls_security_fix.sql.\x1b[0m\n`
);
